...
@export var default_distance: DistanceAlgorithm = DistanceAlgorithm.MANHATTAN

## 获取以指定坐标为中心，指定范围内的所有坐标
## algorithm: 距离算法，
## 支持通过枚举或者直接传入int值
func get_range_cells(center_pos: Vector2i, range_val: int,\
algorithm: DistanceAlgorithm = default_distance) -> Array[Vector2i]:
...
## shape: 形状类型
func get_directional_range_cells(start_pos: Vector2i, direction: Vector2i,\
range_size: Vector2i, shape: ShapeType) -> Array[Vector2i]:
	# 防止方向为零向量
